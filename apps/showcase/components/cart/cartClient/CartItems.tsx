        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-primary">
              {t('productsList')} ({totalItems}{' '}
              {totalItems === 1 ? t('product') : t('product') + 's'})
            </h3>
            <Button
              variant="secondary"
        ...
        {!isOpen && (
          ...
          >
            <motion.div
              ...
              className="flex flex-col gap-4 rounded-lg border border-border bg-white p-4 shadow-sm"
            >
              {items.map((item) => (
                <div className="flex flex-row gap-8" key={item.product.id}>
                  {/* Product Image */}
